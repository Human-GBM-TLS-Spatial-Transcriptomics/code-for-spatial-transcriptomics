...
######Figure 4B
svg("Figure4B_UMAP_all_annotated_new_color_white.svg",width = 8,height = 6)
...
p1<-ImageDimPlot(nano.obj.with.label.transfer, fov = "STGG1_fov5",size = 0.5, coord.fixed = FALSE,axes = T,
...
STGG1.fov5.id<-as.vector(nano.obj.with.label.transfer@images$STGG1_fov5@boundaries$centroids@cells)
STGG1.fov5.obj<-subset_opt(nano.obj.with.label.transfer, cells =STGG1.fov5.id)

DimPlot(STGG1.fov5.obj)
table(STGG1.fov5.obj@meta.data$celltype.new)
...
STGG1.fov5.cell.frac<-data.frame(table(STGG1.fov5.obj@meta.data$celltype.new))
STGG1.fov5.cell.frac$FOV<-"T-TLS"
STGG1.fov5.cell.frac<-STGG1.fov5.cell.frac[-c(3,5,7,9),]

colnames(STGG1.fov5.cell.frac) = c('Celltype','Number of cell','Group')
STGG1.fov5.cell.frac<-STGG1.fov5.cell.frac[order(STGG1.fov5.cell.frac$`Number of cell`,decreasing = F),]
STGG1.fov5.cell.frac$Celltype<-factor(STGG1.fov5.cell.frac$Celltype,levels = STGG1.fov5.cell.frac$Celltype)


p2<-ggplot(STGG1.fov5.cell.frac)+geom_col(aes(x=Celltype,y=STGG1.fov5.cell.frac$`Number of cell`,fill=Celltype),width = 0.8)+
...
svg("Figure4C_STGG1_fov5_with_cell_fraction.svg",width = 8,height = 6)
p1+p2+plot_layout(widths = c(5, 1))
dev.off()

######Figure 4D
p3<-ImageDimPlot(nano.obj.with.label.transfer, fov = "STGU1_fov8",size = 0.5, coord.fixed = FALSE,axes = T,
                 cells = WhichCells(nano.obj.with.label.transfer,idents = c("T cell","Endothelial","B cell","Myeloid cell","Fibroblast")),
...
STGU1.fov8.obj<-subset_opt(nano.obj, cells =STGU1.fov8.id)

DimPlot(STGU1.fov8.obj)
table(STGU1.fov8.obj@meta.data$celltype.new)
B cell  Endothelial  Erythrocyte   Fibroblast   Mural cell Myeloid cell   Neutrophil       T cell   Tumor cell 
1107           26            1           99           13          593            3          296          880 

STGU1.fov8.cell.frac<-data.frame(table(STGU1.fov8.obj@meta.data$celltype.new))
STGU1.fov8.cell.frac<-STGU1.fov8.cell.frac[-c(3,5,7,9),]
STGU1.fov8.cell.frac$FOV<-"B-TLS"

colnames(STGU1.fov8.cell.frac) = c('Celltype','Number of cell','Group')
STGU1.fov8.cell.frac<-STGU1.fov8.cell.frac[order(STGU1.fov8.cell.frac$`Number of cell`,decreasing = F),]
STGU1.fov8.cell.frac$Celltype<-factor(STGU1.fov8.cell.frac$Celltype,levels = STGU1.fov8.cell.frac$Celltype)

p4<-ggplot(STGU1.fov8.cell.frac)+geom_col(aes(x=Celltype,y=STGU1.fov8.cell.frac$`Number of cell`,fill=Celltype),width = 0.8)+
  fraction_plot_theme+coord_flip()+ylab("Number of cells")+NoLegend()+theme(plot.margin = margin(0,0,0,-1))+
  scale_fill_manual(values = c("red","white","cyan","#AC844B","magenta"))

svg("Figure4D_STGU1_fov8_with_cell_fraction.svg",width = 8,height = 6)
p3+p4+plot_layout(widths = c(5, 1))
dev.off()

######Figure 4E 4G 4I
svg("Figure4E_CCL5_expression_in_situ.svg",width = 18,height = 6)
ImageFeaturePlot(nano.obj.with.label.transfer, fov = c("STGG1_fov5","STGU1_fov8"), 
                 ...
                 features = c("CCL5"), min.cutoff = "q5",max.cutoff = "q95")
dev.off()


svg("Figure4G_CXCR4_expression_in_situ.svg",width = 18,height = 6)
ImageFeaturePlot(nano.obj.with.label.transfer, fov = c("STGG1_fov5","STGU1_fov8"), 
                 cells = WhichCells(nano.obj,idents = c("T cell","Endothelial","B cell","Myeloid cell","Fibroblast")),
                 features = c("CXCR4"), min.cutoff = "q5",max.cutoff = "q95")
dev.off()


svg("Figure4I_IL7R_expression_in_situ.svg",width = 18,height = 6)
ImageFeaturePlot(nano.obj.with.label.transfer, fov = c("STGG1_fov5","STGU1_fov8"), 
                 cells = WhichCells(nano.obj,idents = c("T cell","Endothelial","B cell","Myeloid cell","Fibroblast")),
...
g.select<-c("CCL5","CXCR4","IL7R")
svg("Figure4FHJ_2FOV_gene_barplot.svg",8,10)
par(mfrow=c(5,1))
par(mar=c(5,4,2,3))
par(oma = c(1, 1, 1, 1.1))
